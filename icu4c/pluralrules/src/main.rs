...
#[link(name = "icui18n")]
extern "C" {
    pub fn uplrules_open_67(locale: *const c_char, UErrorCode: *mut libc::c_int) -> *mut libc::c_void;
    pub fn uplrules_close_67(uplrules: *mut libc::c_void);
    pub fn uplrules_select_67(uplrules: *mut libc::c_void, number: libc::c_double, keyword: *mut u16, capacity: libc::c_int, UErrorCode: *mut libc::c_int) -> libc::c_int;
...
impl HarnessPluralsRuntime for IcuPluralRules {
    ...
    fn select(&self, values: &[isize]) -> Vec<String> {
        for langid in &self.langids {
            ...
            let mut err = 0;
            let pr = unsafe { uplrules_open_67(lid.as_ptr(), &mut err) };

            for value in values {
                unsafe {
                    // let capacity = uplrules_select_67(pr, *value as f64, std::ptr::null_mut(), 0, &mut err);
                    // // check error.
                    let mut storage = vec![0u16; 255]; // May need +1? Not sure if icu null-terminates
                    let new_capacity = uplrules_select_67(pr, *value as f64, storage.as_mut_ptr(), 255, &mut err);
                ...
                //     let capacity = 255;
                //     let _ = uplrules_select_67(pr, *value as f64, keyword.as_mut_ptr(), capacity, &mut err);
                //     let r = std::ffi::CStr::from_ptr(keyword.as_ptr()).to_string_lossy();
                //     println!("{:#?}", r);
                // }
            }
            unsafe { uplrules_close_67(pr) };
        }
        vec![]
    }
}
